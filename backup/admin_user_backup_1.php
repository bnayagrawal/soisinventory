...
<html lang="en">
	...
	<body onresize="setModalResultHeight()">
		...
		<section>
				...
				<div class="row">
					...
					<?php
						echo "<ul class='collection z-depth-1'>
								<li class='collection-header'><h4 class='<?php echo $theme; ?>-text text-darken-3'>Students</h4></li>";
                        $info = mysqli_query($mysqli, "select user_username, user_password, user_name, user_email, user_phone, user_image, course_id from user join student on user.user_username = student.student_username");
                        ...
                        while($row=mysqli_fetch_array($info,MYSQLI_ASSOC)) {
							...
							$getCourseName = mysqli_query($mysqli,"SELECT course_name FROM course WHERE course_id=" . $row['course_id']);
							while($gcn=mysqli_fetch_array($getCourseName,MYSQLI_ASSOC))
								$student_course = $gcn['course_name'];
							...
							echo "<li class='collection-item avatar' id='collection-item-{$user_username}'>
									  ...
									  <a href='#' onclick='setUserInfo(\"{$user_username}\",\"student\");' data-activates='dropdown-option-{$user_username}' class='secondary-content dropdown-button'><i class='material-icons delete-link grey-text'>more_vert</i></a>
									</li>";
						...
						echo "<div class='col s12 m12 l6'><ul class='collection z-depth-1'>
								<li class='collection-header'><h4 class='<?php echo $theme; ?>-text text-darken-3'>Faculty</h4></li>";
				...
				<!-- SEARCH WINDOW FOR ALL SCREEN SIZES -->
