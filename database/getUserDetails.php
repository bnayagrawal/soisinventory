<?php
	...
	//requies course_id
	$course_id = $_POST["course_id_post"];
	$record_count = 0;
	
	echo '{"user_info":[';
	
	require_once('config.php');
	$exc = mysqli_query($mysqli,'SELECT user_name,user_phone,user_email,user_image,user_username FROM user WHERE user_username in (SELECT student_username FROM student WHERE course_id='.$course_id.')');
